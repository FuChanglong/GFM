...
def adjust_idx(edge_index, node_idx, full_g, center_idx):
    ...
    x_view = Data(edge_index=edge_index, x=full_g.x[node_idx], y=full_g.y[center_idx],
                  root_n_index=node_idx_map[center_idx])
    return x_view
...
def ego_graphs_sampler(node_idx, data, hop=2, sparse=False):
    ...
    for idx in node_idx.numpy().tolist():
        ...
        g = Data(x=sub_x, edge_index=sub_edge_index, root_n_index=mapping, y=data.y[idx],
                 original_idx=subset)  # note: there we use root_n_index to record the index of target node, because `PyG` increments attributes by the number of nodes whenever their attribute names contain the substring :obj:`index`
        g['center_idx'] = idx
        g['neig_idx'] = subset
        ego_graphs.append(g)
    return ego_graphs
